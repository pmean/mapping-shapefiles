...
The census files use a shapefile format. This is a rather complex system with at least three separate files constituting the shapefile. You can find a nice introduction at the Wikipedia page,

https://en.wikipedia.org/wiki/Shapefile

but note that there is some criticism of the shapefile standard,
...
```{r sf}
names(all_districts)
table(all_districts$STATEFP)
all_districts %>%
  ggplot(aes(x=log10(ALAND))) +
  geom_histogram(
    binwidth=0.5, 
    fill="white",
    color="black")
...
```{r last-column}
  la01 <- "2201"
i <- which(all_districts$GEOID==la01)
attributes(all_districts$geometry[i])
attributes(all_districts$geometry[[i]])
print.default(all_districts$geometry[i])
print.default(all_districts$geometry[[i]])
```

We'll start by looking at a congressional district in southern Louisiana, mostly because it has an interesting shape.

Use the st_coordinates function to convert the geometry into lon/lat pairs. The various pieces of the Congressional district are identified by L1, L2, and L3. You can use L1 to identify whether the boundary in the multipolygon represents the exterior or an interior "hole." You can use L2 to identify the sequence of polygon in the multipolygon or of line in the multiline. If you have a mix of multipoygons and multilines, then you can use L3 to identify which is which.

The tables show that there is only one simple feature (because L3 always equals 1), that figure, a multipolygon, consists of a whole bunch of individual polygons (because L2 has many values), and there are no holes in any of the individual polygons (because L1 always equals 1).
...
all_districts                          %>%
  ...
  divide_by(1000)                      %>%
  round                                %>%
  format(big.mark=",")                 -> perim
"District "                            %>%
  paste0(la01)                         %>%
  paste0(" has a perimeter of ")       %>%
  paste0(perim)                        -> title_string
all_districts                          %>%
  filter(GEOID==la01)                  %>%
  ggplot()                              +
  geom_sf(fill="gray")                  +
...
Also note that the mapping system split some of the Aleutian Islands from the rest of Alaska and placed them on the opposite side of the map. That's because there is a dicontinuous transition from 180 degrees east longitude to 180 degrees west longitude (which is stored internally as -180). There's an easy workaround for this, which I will try to find and include here.
...
A nice overview of box counting as a measure of fractal dimension is on Wikipedia (https://en.wikipedia.org/wiki/Minkowski%E2%80%93Bouligand_dimension).

The st_relate function computes the "DE9-IM relation between a pair of geometries." This is a very flexible way to compare two geometric features. It examines the dimensionality of the intersection of the interior/boundary/exterior of one geometric feature with the interior/boundary/exterior of another geometric feature. The possible values are 0, 1, 2, and F where F represents the empty set. A nice explanaiton of DE-9IM is at https://connect.boundlessgeo.com/docs/suite/4.6/dataadmin/pgAdvanced/de9im.html.
...
for (district_number in worst_districts[1]) {
