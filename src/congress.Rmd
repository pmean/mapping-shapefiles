...
```{r count-boxes}
count_boxes <- function(full_map, draw_graph=TRUE) {
...
for (i in 1:n_districts) {
  ...
  fn <- paste0("~/mapping-shapefiles/results/d", district_number, "_%02d.bmp")
  bmp(file=fn)
  bc <- count_boxes(full_map)
  print(bc)
  plot(1:10, log2(bc))
  log_fit <- lm(log2(bc)~c(1:10))
  abline(log_fit)
  title(round(coef(log_fit)[2],2))
  print(log_fit)
  dev.off()
  fn <- "~/mapping-shapefiles/results/all_districts.csv"
  write.csv(data.frame(d=district_number, x=1:10, y=bc), file=fn, append=TRUE)
}
```
