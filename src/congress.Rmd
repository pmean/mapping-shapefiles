...
full_map[2, ] %>%
  ...
  st_coordinates -> ce
plot(full_map)
text(sw_corner(full_map),"SW")
text(se_corner(full_map),"SE")
text(nw_corner(full_map),"NW")
text(ne_corner(full_map),"NE")
text(map_middle(full_map), "MIDDLE")
distm(sw_corner(full_map), se_corner(full_map))
distm(nw_corner(full_map), ne_corner(full_map))
distm(sw_corner(full_map), nw_corner(full_map))
distm(se_corner(full_map), ne_corner(full_map))
box_vertices <- matrix(-1, 5, 2)
box_vertices[1,  ] <- as.matrix(sw_corner(full_map))
box_vertices[3,  ] <- destPoint(box_vertices[1, ],  45, 100000*sqrt(2))
...
print(box_vertices)
print(se_corner(full_map))
dimnames(box_vertices) <- list(NULL, c("x", "y"))
lines(box_vertices)

box_vertices[1,  ] <- box_vertices[3,  ]
box_vertices[3,  ] <- destPoint(box_vertices[1, ],  45, 100000*sqrt(2))
box_vertices[5,  ] <- box_vertices[1,  ]
...
dimnames(box_vertices) <- list(NULL, c("x", "y"))
lines(box_vertices)
st_crosses(st_linestring(box_vertices), full_map[2, ], sparse=FALSE)
st_relate(st_linestring(box_vertices), full_map[2, ]) # FF1FFF1F2
st_relate(full_map[2, ], st_linestring(box_vertices))
st_relate(full_map[1, ], st_linestring(box_vertices))

```

```{r more-cases, eval=FALSE}
